...
import {
  LayoutDashboard, History, User, LogOut, Activity,
  ChevronRight, ChevronDown, FileText, Shield,
  BarChart3, Clock, Apple, BadgeCheck, Database,
  QrCode, FileSearch, Settings, HelpCircle, BookOpen,
  Bell, Search, ChevronLeft, Loader2, Menu, X
} from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const [expandedGroup, setExpandedGroup] = useState<string | null>('analysis');
  ...
  const navSections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        { path: '/app?tab=interactions', icon: Shield, label: 'Drug Interactions', badge: 'Live', badgeColor: 'bg-red-50 text-red-600' },
        { path: '/app?tab=dosage', icon: BadgeCheck, label: 'Dosage Validation', },
        { path: '/app?tab=food', icon: Apple, label: 'Dietary Restrictions', },
        { path: '/app?tab=insurance', icon: Database, label: 'Insurance Mapping', badge: 'CGHS', badgeColor: 'bg-blue-50 text-blue-600' },
      ]
    },
    {
      title: 'Patient',
      items: [
        ...
        { path: '/profile#qr', icon: QrCode, label: 'Emergency Card', badge: 'QR', badgeColor: 'bg-violet-50 text-violet-600' },
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className={`h-16 flex items-center border-b border-gray-100 shrink-0 ${sidebarCollapsed ? 'px-4 justify-center' : 'px-5 justify-between'}`}>
        {!sidebarCollapsed && (
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-gray-900 text-base">MedLens</span>
          </div>
        )}
        <button
              ...
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    `}
                  >
                    <item.icon className={`w-4 h-4 shrink-0 ${active ? 'text-gray-900' : 'text-gray-400 group-hover:text-gray-600'}`} />
                    {!sidebarCollapsed && (
                      <>
                        <span className="text-sm font-medium flex-1 truncate">{item.label}</span>
                        {item.badge && (
                          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${item.badgeColor || 'bg-gray-100 text-gray-500'}`}>
                            {item.badge}
                          </span>
                        )}
                        {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-gray-900 rounded-r-full" />}
                      </>
                    )}
                  </Link>
                );
      ...
      {/* User Footer */}
      <div className={`shrink-0 border-t border-gray-100 ${sidebarCollapsed ? 'p-3' : 'p-3'}`}>
        {!sidebarCollapsed ? (
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 flex flex-col selection:bg-violet-100">
      {/* Top Header */}
      <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 z-30 shrink-0 fixed inset-x-0 top-0">
      ...
      <div className="flex flex-1 pt-14">
        ...
        {/* Sidebar */}
        <aside className={`
          fixed top-14 bottom-0 left-0 z-30 bg-white border-r border-gray-200 flex flex-col
        ...
        <main className={`flex-1 flex flex-col min-h-[calc(100vh-3.5rem)] transition-all duration-300 ${sidebarCollapsed ? 'lg:pl-16' : 'lg:pl-60'}`}>
          <div className="flex-1 p-6 md:p-8 max-w-[1400px] w-full mx-auto animate-fadeSlideUp">
            {children}
          </div>
